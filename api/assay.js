...
export default async function handler(req, res) {
  ...
  const baseLimit = TIER_LIMITS[profile.tier] || TIER_LIMITS.Free;
  const extra = (profile.extra_assays && profile.usage_period === period) ? profile.extra_assays : 0;
  const totalLimit = profile._unlimited ? 999999 : baseLimit + extra;

  if (used >= totalLimit) {
    return res.status(403).json({
      error: 'Monthly limit reached (' + profile.tier + ': ' + totalLimit + '). Please upgrade or top up to continue.'
    });
  }
